...
- **One spec**, one flag gate: entire Phase 3 UX is behind existing `job-detail-fullscreen-edit`. Flag off keeps today’s Untitled-Job create, Inbox tap-to-assign, Quick Material qty+unit-cost save gate, and the legacy live overlay (header **EDIT**, expandable session card, add tiles, nested sheets).
- **New Job composer** (Jobs FAB, primary **+ → New Job**, Home empty first-job — same surface):
  ...
  - Progressive reveal: control(s) append the **same Job Edit white tiles** (customer, address, revenue, long description, Add session / material / other cost / note) into the growing sheet. In-memory until **Add Job**.
...
- **Live Session — View start tile:** In Sessions section when **this job has no in-progress session and the user has no live session anywhere**: `Live Session` / `Start a timer now`. Uses existing `startLiveSession` path and **closes job detail**. Hide tile whenever any live session exists (minimized bar is the way back). FAB **Live Session** remains.
- **Live Session — overlay (flag on):**
...
| REQ-F02 | Flag off: Untitled-Job create + `initialEditOpen`, Inbox tap-to-assign, legacy Quick Material save gate, legacy live overlay unchanged. | TEST-F02 |
...
| REQ-N04 | Flag on: Composer can expand to Job Edit white tiles before **Add Job**; draft is in-memory until commit. | TEST-N04 |
...
| REQ-L07 | Flag on: Start time editable in place on overlay; no `EditLiveSessionBottomSheet`. **End Session** ends session. | TEST-L07 |
| REQ-L08 | Flag on: No header **EDIT**, no nested `EditJobBottomSheet`, no second **Done** on live overlay. | TEST-L08 |
...
- Compatibility: flag off is full legacy; flag on requires mobile build that implements Phase 3; no DB migration required for core behavior (optional `createJobForCurrentUser` helper is additive).
